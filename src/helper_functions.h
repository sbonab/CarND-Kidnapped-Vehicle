 ...
 * @output A vector of same size of input vector but normalized
 */
inline bool normalize(std::vector<double> &vec) {
  double mag = 0;
  for (auto v:vec){
    mag += v;
  }
  if (mag == 0){
    throw std::logic_error("Magintude of vector is zero");
    return false;
  }
 ...
 * @param (sig_x,sig_y) standard deviation in x and y direction
 * @param (x_obs,y_obs) x and y coordinates of the observation
 * @param (mu_x,mu_y) x and y coordinates of the associated map landmark
